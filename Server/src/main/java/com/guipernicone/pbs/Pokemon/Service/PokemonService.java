...
//			System.out.println();
	...
	public Page<Pokemon> getPokemonsStatusList(int pageNumber, int size) {
		Pageable page = PageRequest.of(pageNumber, size, Direction.ASC, "pokedexNumber");
		
		Page<Pokemon> pokemons = pokemonRepository.findAll(page);
		
		if (pokemons.hasContent()) {
			return pokemons;
		}
		else {
			return null;
